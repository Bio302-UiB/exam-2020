...
Disadvantages:
 ...
 - limits flexibility (but can deviate from pre-registered plan but need to justify)
 - exciting new finding cannot be discussed (they can, just need to be labelled as _post hoc_)
 - unsuitable for some types of work (purely exploratory; urgent work e.g. after an unexpected event that needs researching)

Also:
 
- PARKing possible (pre-registration after results known - but this is misconduct)
...
- effect size is a more useful statistic.
...
- test if adding autocorrelation to model (gls or lme) improves the model
...
Multiple ways to process data. ~All code in tidyverse solution below was covered in the course. None tidyverse solutions were accepted provided they were reproducible. 
...
# here::here is particularly useful in Rmd files which might not in the project root directory
...
#this generates a lot of warnings for sheet 4 - not obvious why.
...
biomass %>% ggplot(aes(x = temp, y = biomass, fill = site)) + geom_boxplot()
```
Grouped data. Need to use mixed effect model (or as number of levels in random effect is low, perhaps a ordinary least squares model with site as fixed effect).
...
ggplot(biomass, aes(x = temp, y = biomass)) + 
  geom_point(aes(colour = site), alpha = 0.5, position = position_jitter(height = 0, width = 0.1)) + #jitter *slightly*
  geom_line(data = pred) + 
  #change fill to have meaningful colours
